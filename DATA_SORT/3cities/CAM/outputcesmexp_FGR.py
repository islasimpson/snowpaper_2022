...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/FGR/*.nc"
    print(fpath)
    ...
    for icity in np.arange(0,len(cityname),1):
       fgr[:,icity] = dat.FGR.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    fgr.to_netcdf(path=pathout+"FGR_"+expname[iexp]+".nc")
