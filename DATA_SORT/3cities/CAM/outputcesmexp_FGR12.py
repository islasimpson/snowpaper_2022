...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/FGR12/*.nc"
    print(fpath)
    dat = read.read_sfc_cesm(fpath,"1979-01-01","2014-12-31")
    
    fgr = xr.DataArray(np.zeros([dat.time.size, 3]), coords=[dat.time, cityname],
                                       dims=['time','city'], name='fgr12')
    
    for icity in np.arange(0,len(cityname),1):
       fgr[:,icity] = dat.FGR12.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    fgr.to_netcdf(path=pathout+"FGR12_"+expname[iexp]+".nc")
