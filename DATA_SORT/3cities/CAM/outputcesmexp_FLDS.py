...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/FLDS/*.nc"
    print(fpath)
    dat = read.read_sfc_cesm(fpath,"1979-01-01","2014-12-31")
    
    flds = xr.DataArray(np.zeros([dat.time.size, 3]), coords=[dat.time, cityname],
                                       dims=['time','city'], name='flds')
    
    for icity in np.arange(0,len(cityname),1):
       flds[:,icity] = dat.FLDS.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    flds.to_netcdf(path=pathout+"FLDS_"+expname[iexp]+".nc")
