...
#expname=[ 'Isla_CAM6_CLM5', "CAM6_CLM5_snowdensity" ]
...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/3h/FSNS/*.nc"
    print(fpath)
    ...
    fsns = xr.DataArray(np.zeros([timeout.size, 3]), coords=[timeout, cityname],
                                       dims=['time','city'], name='fsns')
    
    for icity in np.arange(0,len(cityname),1):
       fsns[:,icity] = dat.FSNS.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    fsns.to_netcdf(path=pathout+"FSNS_"+expname[iexp]+".nc")
