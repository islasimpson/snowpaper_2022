...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/LHFLX/*.nc"
    print(fpath)
    ...
    for icity in np.arange(0,len(cityname),1):
       lhflx[:,icity] = dat.LHFLX.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    lhflx.to_netcdf(path=pathout+"LHFLX_"+expname[iexp]+".nc")
