...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/SNOWDP/*.nc"
    print(fpath)
    ...
    for icity in np.arange(0,len(cityname),1):
       snowdp[:,icity] = dat.SNOWDP.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    snowdp.to_netcdf(path=pathout+"SNOWDP_"+expname[iexp]+".nc")
