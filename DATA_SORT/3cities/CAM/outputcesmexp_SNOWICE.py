...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/SNOWICE/*.nc"
    print(fpath)
    ...
    for icity in np.arange(0,len(cityname),1):
       snowice[:,icity] = dat.SNOWICE.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    snowice.to_netcdf(path=pathout+"SNOWICE_"+expname[iexp]+".nc")
