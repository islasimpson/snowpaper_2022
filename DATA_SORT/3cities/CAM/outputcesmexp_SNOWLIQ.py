...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/SNOWLIQ/*.nc"
    print(fpath)
    ...
    for icity in np.arange(0,len(cityname),1):
       snowliq[:,icity] = dat.SNOWLIQ.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    snowliq.to_netcdf(path=pathout+"SNOWLIQ_"+expname[iexp]+".nc")
