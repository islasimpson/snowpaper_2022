...
expname=['CAM6_CLM5_snowdensity','CAM6_CLM5_snowdensity_002','Isla_CAM6_CLM5','Isla_CAM6_CLM5_002']
...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/T850/*.nc"
    print(fpath)
    ...
    for icity in np.arange(0,len(cityname),1):
       t850[:,icity] = dat.T850.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    t850.to_netcdf(path=pathout+"T850_"+expname[iexp]+".nc")
