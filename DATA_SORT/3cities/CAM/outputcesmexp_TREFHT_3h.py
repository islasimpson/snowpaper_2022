...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/3h/TREFHT/*.nc"
    print(fpath)
    ...
    for icity in np.arange(0,len(cityname),1):
       trefht[:,icity] = dat.TREFHT.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    trefht.to_netcdf(path=pathout+"TREFHT_3hourly_"+expname[iexp]+".nc")
