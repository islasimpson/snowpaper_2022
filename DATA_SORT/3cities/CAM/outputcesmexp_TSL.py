...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/TSL/*.nc"
    print(fpath)
    ...
    for icity in np.arange(0,len(cityname),1):
       tsl[:,icity] = dat.TSL.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    tsl.to_netcdf(path=pathout+"TSL_"+expname[iexp]+".nc")
