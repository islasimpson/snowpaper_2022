...
expname=['Isla_CAM6_CLM5_002','CAM6_CLM5_snowdensity','CAM6_CLM5_snowdensity_002']
...
for iexp in np.arange(0,len(expname),1):
    ...
    fpath=basedir+expname[iexp]+"/day/CLIM_ADVECTION/*.nc"
    print(fpath)
    dat = read.read_sfc_cesm(fpath,"1979-01-01","2014-12-31")
    
    advection = xr.DataArray(np.zeros([dat.time.size, 3]), coords=[dat.time, cityname],
                                       dims=['time','city'], name='advection')
    
    for icity in np.arange(0,len(cityname),1):
       advection[:,icity] = dat.advection.sel(lon=citylon[icity],lat=citylat[icity], method='nearest')
    
    advection.to_netcdf(path=pathout+"ADVECTION_"+expname[iexp]+".nc")
