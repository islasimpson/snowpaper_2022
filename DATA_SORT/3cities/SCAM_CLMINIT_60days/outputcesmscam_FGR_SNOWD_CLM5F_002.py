...
for icity in np.arange(0,3,1):
    ...
    if (icity == 0): 
        fgr = xr.DataArray(np.zeros([dat.time.size, 3]), coords=[dat.time, cityname],
                                       dims=['time','city'], name='fgr')
    
    fgr[:,icity] = dat.FGR.isel(lon=0,lat=0)
    
    fgr.to_netcdf(path=pathout+"FGR_"+outname+".nc")
