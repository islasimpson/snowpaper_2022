...
for icity in np.arange(0,3,1):
    ...
    if (icity == 0): 
        flns = xr.DataArray(np.zeros([dat.time.size, 3]), coords=[dat.time, cityname],
                                       dims=['time','city'], name='flds')
    
    flns[:,icity] = dat.FLDS.isel(lon=0,lat=0)
    
    flns.to_netcdf(path=pathout+"FLDS_"+outname+".nc")
