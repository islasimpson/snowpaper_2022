...
for icity in np.arange(0,3,1):
    ...
    if (icity == 0): 
        lhflx = xr.DataArray(np.zeros([dat.time.size, 3]), coords=[dat.time, cityname],
                                       dims=['time','city'], name='lhflx')
    
    lhflx[:,icity] = dat.LHFLX.isel(lon=0,lat=0)
    
    lhflx.to_netcdf(path=pathout+"LHFLX_"+outname+".nc")
