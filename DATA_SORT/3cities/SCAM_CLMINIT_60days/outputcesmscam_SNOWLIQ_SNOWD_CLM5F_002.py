...
expname=['SASK_SNOWD_CLM5F_02.001.FSCAM.sask2_1979_2014',
         'TOR_SNOWD_CLM5F_02.001.FSCAM.tor2_1979_2014',
         'SID_SNOWD_CLM5F_02.001.FSCAM.sid2_1979_2014']

outname='SCAM_SNOWD_CLM5F_02'
...
for icity in np.arange(0,3,1):
    ...
    if (icity == 0): 
        snowliq = xr.DataArray(np.zeros([dat.time.size, 3]), coords=[dat.time, cityname],
                                       dims=['time','city'], name='snowliq')
    
    snowliq[:,icity] = dat.SNOWLIQ.isel(lon=0,lat=0)
    
    snowliq.to_netcdf(path=pathout+"SNOWLIQ_"+outname+".nc")
