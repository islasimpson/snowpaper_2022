...
for icity in np.arange(0,3,1):
    ...
    if (icity == 0): 
        t = xr.DataArray(np.zeros([dat.time.size, dat.lev.size, 3]), coords=[dat.time, dat.lev, cityname],
                                       dims=['time','lev','city'], name='T')
    
    t[:,:,icity] = dat.T.isel(lon=0,lat=0)
    
    t.to_netcdf(path=pathout+"T_"+outname+".nc")
