...
fgr12_clm5 = xr.open_dataset(basepath+"FGR12_Isla_CAM6_CLM5_002.nc")
...
tlowlev_clm5 = xr.open_dataset(basepath+"/tlowlev_Isla_CAM6_CLM5_002.nc")
...
fgr12_snowd = xr.open_dataset(basepath+"FGR12_CAM6_CLM5_snowdensity_002.nc")
...
tlowlev_snowd = xr.open_dataset(basepath+"/tlowlev_CAM6_CLM5_snowdensity_002.nc")

cities = trefht_clm5.city
ncities = trefht_clm5.city.size
nlev = tlowlev_snowd.lev.size
lev = tlowlev_snowd.lev
...
fgr12_clm5_deseas = calcdeseas(fgr12_clm5.fgr12)
...
tlowlev_clm5_deseas = calcdeseas(tlowlev_clm5.T)
...
fgr12_snowd_deseas = calcdeseas(fgr12_snowd.fgr12)
...
tlowlev_snowd_deseas = calcdeseas(tlowlev_snowd.T)
...
for icity in range(0,ncities,1):
    ...
    fgr12_clm5 = np.array(fgr12_clm5_deseas[:,:,icity]).flatten()
    fgr12_snowd = np.array(fgr12_snowd_deseas[:,:,icity]).flatten()
    ...
    tlowlev_clm5 = np.array(tlowlev_clm5_deseas[:,:,:,icity])
    tlowlev_snowd = np.array(tlowlev_snowd_deseas[:,:,:,icity])
    ...
    if (icity == 0):
        ...
        fgr12comp_clm5 = np.zeros([nblocks, ncities])
        fgr12comp_snowd = np.zeros([nblocks, ncities])
        ...
        tlowlevcomp_clm5 = np.zeros([nblocks,nlev,ncities])
        tlowlevcomp_snowd = np.zeros([nblocks,nlev,ncities])
    ...
    for iblock in np.arange(0,nblocks,1):
        ...
        fgr12comp_clm5[iblock, icity] = \
         (fgr12_clm5[ (trefht_clm5 >= binmin[iblock]) & (trefht_clm5 < binmax[iblock])]).mean()

        fgr12comp_snowd[iblock, icity] = \
         (fgr12_snowd[ (trefht_snowd >= binmin[iblock]) & (trefht_snowd < binmax[iblock])]).mean()
        ...
        for ilev in np.arange(0,nlev,1):
            datclm5 = tlowlev_clm5[:,:,ilev].flatten()
            tlowlevcomp_clm5[iblock,ilev,icity] = np.nanmean(
             datclm5[(trefht_clm5 >= binmin[iblock]) & (trefht_clm5 < binmax[iblock])])

            datsnowd = tlowlev_snowd[:,:,ilev].flatten()
            tlowlevcomp_snowd[iblock,ilev,icity] = np.nanmean(
...
fgr12_clm5_xr = xr.DataArray(fgr12comp_clm5, 
                 coords=[np.arange(0,nblocks,1),cities], dims=['ptile','city'], name='fgr12_clm5')
...
tlowlev_clm5_xr = xr.DataArray(tlowlevcomp_clm5,
...
fgr12_snowd_xr = xr.DataArray(fgr12comp_snowd, 
                    coords=[np.arange(0,nblocks,1),cities], dims=['ptile','city'], name='fgr12_snowd')
...
tlowlev_snowd_xr = xr.DataArray(tlowlevcomp_snowd,
                    coords=[np.arange(0,nblocks,1),lev,cities], dims=['ptile','lev','city'],
                    name='tlowlevs_snowd')
...
fgr12_clm5_xr.to_netcdf(path=outpath+"trefhtptilecomposites_3cities.nc", mode="a")
...
tlowlev_clm5_xr.to_netcdf(path=outpath+"trefhtptilecomposites_3cities.nc", mode="a")
...
fgr12_snowd_xr.to_netcdf(path=outpath+"trefhtptilecomposites_3cities.nc", mode="a")
...
tlowlev_snowd_xr.to_netcdf(path=outpath+"trefhtptilecomposites_3cities.nc", mode="a")
